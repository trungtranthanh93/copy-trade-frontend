  ...
  methods: {
    ...
    async getMontlyExport() {
      try {
        ...
        let data = await api.get(
          `/users/${this.userId}/monthly-export/${this.monthly + 1}`
        );
        this.totalMonth = data.data.totalIncome
          ? parseFloat(data.data.totalIncome).toFixed(2)
          : 0;
    ...
    async onPrev() {
      ...
      if (
        new Date(this.selectedDate).getFullYear() !== new Date().getFullYear()
      ) {
